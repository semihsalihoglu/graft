...
public class GiraphVertexScenarioWrapper<I extends WritableComparable, V extends Writable,
  ...
  public class VertexContextWrapper extends BaseWrapper {
    ...
    public void setVertexValueBeforeWrapper(V vertexValueBefore) {
      ...
      // later).
      this.vertexValueBeforeWrapper = DebugUtils.makeCloneOf(vertexValueBefore,
    ...
    public void setVertexValueAfterWrapper(V vertexValueAfter) {
      ...
      this.vertexValueAfterWrapper = DebugUtils.makeCloneOf(vertexValueAfter,
        getVertexScenarioClassesWrapper().vertexValueClass);
    }
    ...
    public void addIncomingMessageWrapper(M1 message) {
      // See the explanation for making a clone inside
      // setVertexValueBeforeWrapper
      inMsgsWrapper.add(DebugUtils.makeCloneOf(message, getVertexScenarioClassesWrapper().incomingMessageClass));
    }
    ...
    public void addOutgoingMessageWrapper(I receiverId, M2 message) {
      // See the explanation for making a clone inside
      // setVertexValueBeforeWrapper
      outMsgsWrapper.add(new OutgoingMessageWrapper(DebugUtils.makeCloneOf(receiverId,
        getVertexScenarioClassesWrapper().vertexIdClass),
        DebugUtils.makeCloneOf(message, getVertexScenarioClassesWrapper().outgoingMessageClass)));
    }
    ...
    public void addNeighborWrapper(I neighborId, E edgeValue) {
      ...
      neighborsWrapper.add(new NeighborWrapper(
        DebugUtils.makeCloneOf(neighborId, getVertexScenarioClassesWrapper().vertexIdClass),
        DebugUtils.makeCloneOf(edgeValue, getVertexScenarioClassesWrapper().edgeValueClass)));
    }
    ...
    public class NeighborWrapper extends BaseWrapper {
      ...
      public void loadFromProto(GeneratedMessage protoObject) throws ClassNotFoundException,
        ...
        this.nbrId = DebugUtils.newInstance(vertexScenarioClassesWrapper.vertexIdClass);
        fromByteString(neighbor.getNeighborId(), this.nbrId);

        if (neighbor.hasEdgeValue()) {
          this.edgeValue = DebugUtils.newInstance(vertexScenarioClassesWrapper.edgeValueClass);
    ...
    public class OutgoingMessageWrapper extends BaseWrapper {
      ...
      public void loadFromProto(GeneratedMessage generatedMessage) throws ClassNotFoundException,
        ...
        this.destinationId = DebugUtils.newInstance(getVertexScenarioClassesWrapper().vertexIdClass);
        fromByteString(outgoingMessageProto.getDestinationId(), destinationId);
        this.message = DebugUtils.newInstance(getVertexScenarioClassesWrapper().outgoingMessageClass);
    ...
    public void loadFromProto(GeneratedMessage generatedMessage) throws ClassNotFoundException,
      ...
      I vertexId = DebugUtils.newInstance(getVertexScenarioClassesWrapper().vertexIdClass);
      fromByteString(context.getVertexId(), vertexId);
      this.vertexIdWrapper = vertexId;

      V vertexValueBefore = DebugUtils.newInstance(getVertexScenarioClassesWrapper().vertexValueClass);
      ...
      if (context.hasVertexValueAfter()) {
        V vertexValueAfter = DebugUtils.newInstance(getVertexScenarioClassesWrapper().vertexValueClass);
      ...
      for (int i = 0; i < context.getInMessageCount(); i++) {
        M1 msg = DebugUtils.newInstance(getVertexScenarioClassesWrapper().incomingMessageClass);
